...
def get_argparse():
    ...
    ##数据集
    parser.add_argument("--data_dir", default="datasets/35wError", type=str,
    ...
    ## 预训练模型路径&名称
    parser.add_argument("--model_name_or_path", default="prev_trained_model/electra_zh_small", type=str,
    ...
    parser.add_argument("--config_name", default="prev_trained_model/electra_zh_small/config.json", type=str,
                        help="【配置文件】Pretrained config name or path if not the same as model_name")

    parser.add_argument("--tokenizer_name", default="prev_trained_model/electra_zh_small/vocab.txt", type=str,
                        help="【词表路径vocab.txt】Pretrained tokenizer name or path if not the same as model_name", )

    parser.add_argument("--contiu_train", default="outputs/electra_small_35wnltk&js/checkpoint-13100 acc: 0.8940 - recall: 0.8485 - f1: 0.8707", type=str,
                        help="从上一步保存开始训练", )
