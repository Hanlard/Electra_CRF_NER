...
from models.albert_for_ner import AlbertCrfForNer,AlbertLSTMCrfForNer
...
MODEL_CLASSES = {
    ...
    'albert_lstm': (AlbertConfig, AlbertLSTMCrfForNer, CNerTokenizer),
    'electra':(ElectraConfig, ElectraCrfForNer, CNerTokenizer),
}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        for i, label in enumerate(out_label_ids):
            ## 修复
            if args.fix:
                label = fix(input_ids[i],label,args.id2label,tokenizer)
            temp_1 = []
            temp_2 = []
            for j, m in enumerate(label):
                if j == 0:
                    ...
                    temp_1.append(args.id2label[out_label_ids[i][j]])#标签
                    temp_2.append(args.id2label[tags[i][j]])#预测
        pbar(step)
    logger.info("\n")
    eval_loss = eval_loss / nb_eval_steps
    eval_info, entity_info,(origin, found, right) = metric.result(return_num=True)
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        if args.fix:
            preds = fix(input_ids, preds, args.id2label, tokenizer)
...
def main():
    ...
    if args.model_type.lower()=="electra":
        model.bert=ElectraModel.from_pretrained(args.model_name_or_path)
        logger.info(f"Loading Electra from {args.model_name_or_path}...")

    if args.contiu_train:
        model = model_class.from_pretrained(args.contiu_train, config=config)
        print(f"Loading model from {args.contiu_train}")
    ...
    # Training
    # map {'X': 0, 'O': 1, '[START]': 2, '[END]': 3, 'B-ORG': 4, 'I-ORG': 5}
    if args.do_train:
